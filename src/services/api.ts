...
export const signupAPI = async ({ userId, password, username }: User): Promise<APIResponse> => {
  try {
    ...
    if (error instanceof AxiosError && error.response) {
      if (error.response.data.message === 'UserId is duplicated')
        return {result: 'OK', focus : 'userId', errorMessage : '입력된 아이디는 이미 가입된 상태입니다.'}
    }
    return {result: 'error', focus: '', errorMessage: '네트워크 에러'};
  }
};

export const loginAPI = async ({userId, password}: Omit<User, 'username'>): Promise<APIResponse> => {
  try {
    await axios.post(`${config.backendUrl}/login`, {}, {withCredentials: true, params: {
      id: userId,
      password,
    }})
    return {result: 'OK', focus: '', errorMessage: ''};
    ...
    if (error instanceof AxiosError && error.response) { 
      if (error.response.data.message === '아이디가 잘못되었습니다.') 
        return {result: 'error', focus: 'userId', errorMessage: '입력하신 아이디가 옳지 않습니다.'}
  }
